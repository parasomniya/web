...
> Файл: `emulator.js` | Версия: 2.2 | Node.js 18+
...
| `authToken` | `String` | JWT для загрузки защищённых зон через `/api/telemetry/zones` |
...
2. Если передан `--token` или `AUTH_TOKEN`, добавляет `Authorization: Bearer <token>`
3. Выполняет `GET`-запрос
4. Фильтрует зоны: `active !== false`
5. Логирует результат и возвращает массив

**Ошибки:** При `401/403` явно подсказывает, что нужен JWT-токен.
...
**Последовательность:**
1. **Потеря связи:** С вероятностью `signalLossChance` устанавливает `gpsValid: false` на случайный интервал. В этот период координаты принудительно `0,0`, а после окончания окно связи восстанавливается.
...
| `--token=<JWT>` | ❌ | — | JWT для `GET /api/telemetry/zones` |
...
### Базовый запуск
```bash
node emulator.js --mode=auto --host=http://localhost:3000 --tractor-id=TR-01 --token=<JWT>
```
...
# Терминал 2
node emulator.js --mode=auto --host=http://localhost:3000 --tractor-id=TR-02 --token=<JWT>

# Терминал 3
node emulator.js --mode=auto --host=http://localhost:3000 --tractor-id=TR-03 --token=<JWT>
```
